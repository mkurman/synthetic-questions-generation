...
from api.parser import parse_args_with_config
...
async def main():
    ...
    if args.rate_limit_wait < 0:
        logger.error("--rate-limit-wait must be >= 0")
        return
    if args.rate_limit_retries < 0:
        logger.error("--rate-limit-retries must be >= 0")
        return

    # Validate "other" provider requirements
    if args.provider == "other" and not args.provider_url:
        logger.error("--provider-url is required when using --provider other")
        return

    if args.answer_provider == "other" and not args.provider_url:
        logger.error("--provider-url is required when using --answer-provider other")
        return
    ...
    if start_idx >= len(dataset_list):
        logger.error(
            f"Start index {start_idx} is beyond dataset size {len(dataset_list)}"
        )
        return
    ...
    if args.verbose and (start_idx > 0 or end_idx < original_size):
        print(
            f"📊 Selected range: items {start_idx} to {end_idx-1} (total: {len(dataset_list)} items)"
        )
    ...
    # Initialize provider
    try:
        custom_prompt_dir = getattr(args, "custom_prompts", None)
        ...
        if args.verbose:
            ...
            print(
                f"⏳ Rate limit: wait {args.rate_limit_wait}s, retries {args.rate_limit_retries}"
            )
    except Exception as e:
    ...
    if args.with_answer:
        try:
            answer_provider_name = (
                args.answer_provider if args.answer_provider else args.provider
            )
            ...
            answer_custom_url = (
                args.provider_url
                if (answer_provider_name == "other" or args.provider == "other")
                else None
            )
            ...
            if args.verbose:
                print(
                    f"🤖 Initialized answer provider: {answer_provider_name}/{answer_model}"
                )
                ...
                print(
                    f"⏳ Answer rate limit: wait {args.rate_limit_wait}s, retries {args.rate_limit_retries}"
                )
        except Exception as e:
    ...
    style_options_count = sum(
        [bool(args.style), bool(args.no_style), bool(args.styles_file)]
    )

    if style_options_count > 1:
        logger.error(
            "Only one of --style, --no-style, or --styles-file can be specified"
        )
        return
    ...
    if args.verbose:
        ...
        if args.start_index > 0 or args.end_index is not None:
            ...
            print(
                f"📍 Range: items {args.start_index} to {end_display} from original dataset"
            )
        if is_local_file(args.dataset):
        ...
        if args.with_answer:
            answer_provider_display = (
                args.answer_provider if args.answer_provider else args.provider
            )
            answer_model_display = (
                args.answer_model if args.answer_model else args.model
            )
            print(f"💬 Answer model: {answer_provider_display}/{answer_model_display}")
            print(
                f"📝 Answer mode: {'Single request' if args.answer_single_request else 'One per question'}"
            )
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
        ...
        async def process_with_semaphore(item, index):
            async with semaphore:
                return await process_dataset_item(
                    item, question_generator, session, index, args.text_column
                )
        ...
        if args.with_answer:
            progress_desc = (
                "Generating Q&A" if not args.verbose else "Q&A generation (verbose)"
            )
        else:
            progress_desc = (
                "Generating questions"
                if not args.verbose
                else "Question generation (verbose)"
            )
            ...
            for task in tqdm(
                ...
                try:
                            ...
                            if "error" not in record:
                                ...
                                if args.verbose:
                                    ...
                                    if (
                                        ...
                                        print(
                                            f"✅ Q&A {successful_questions}: {question_preview}... → {answer_preview}..."
                                        )
                                    else:
                                        print(
                                            f"✅ Question {successful_questions}: {question_preview}..."
                                        )
                            else:
                                failed_items += 1
                                if args.verbose:
                                    print(
                                        f"❌ Failed to generate questions: {record['error']}"
                                    )
                    ...
                    if args.sleep_between_items > 0:
                        if args.verbose:
                            print(
                                f"⏱️  Sleeping {args.sleep_between_items}s between items..."
                            )
    ...
    if args.with_answer:
        ...
        print(
            f"📈 Answer success rate: {(successful_answers/successful_questions*100):.1f}%"
            if successful_questions > 0
            else "📈 Answer success rate: 0.0%"
        )
    print(f"❌ Failed items: {failed_items}")
    print(
        f"📈 Question success rate: {(successful_questions/(dataset_size*args.num_questions)*100):.1f}%"
    )
    ...
    if args.with_answer:
        ...
        print(
            f"💬 Answer provider used: {answer_provider_display}/{answer_model_display}"
        )
